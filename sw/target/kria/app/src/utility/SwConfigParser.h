...
struct sw_config{
    ...
    double genoa_pulse_width_us;
    uint32_t genoa_single_neuron_id;
    uint32_t genoa_network_burst_threshold;
    uint32_t genoa_network_burst_window;
    uint32_t genoa_group_burst_threshold;
    uint32_t genoa_group_burst_window;
    std::vector<uint16_t> genoa_group_neuron_id;
    int genoa_group_neuron_id_tab[NB_GROUP_OF_NEURONS_MAX];
};
...
class SwConfigParser{
    ...
    public:
        ...
        void print(std::string key, double value);
        void print(std::string key, uint32_t value);
